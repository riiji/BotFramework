...
namespace Kysect.BotFramework.ApiProviders.Telegram
{
    public class TelegramApiProvider : IBotApiProvider, IDisposable
    {
        ...
        {
            Task<Message> task;
            var stream = System.IO.File.Open(videoPath, FileMode.Open);
            InputMedia fts = new InputMedia(stream,videoPath.Split("\\").Last());
            task = _client.SendVideoAsync(sender.GroupId, fts, caption: text);
            try
            {
                task.Wait();
                stream.Close();
